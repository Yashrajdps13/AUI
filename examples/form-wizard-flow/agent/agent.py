...
- SENSITIVE DATA REDACTION (PII): Any state slot marked as sensitive (e.g. `"sensitive": true` in registry schema) will report its value as `"[REDACTED]"` in the current state values for security. Once you issue a `setState` command on a sensitive slot, it is successfully updated in React, but its reported value in the next step will continue to read as `"[REDACTED]"`. Do NOT assume the state update failed or try to write/re-enter it again if you see `"[REDACTED]"`. Simply treat it as successfully set and proceed.
- COMPLETION/DONE CONDITION: Respond with the word DONE only when the overall goal of the user request is fully achieved. For example, if the goal is to "register" or "complete registration", you must navigate through all the steps of the wizard, select the appropriate tier, accept any necessary terms, and click the final submit button to reach the registration success screen. Once the state shows `step` is `"success"` (or you have reached the registration complete screen), the goal is fully achieved; you MUST immediately respond with the single word DONE and nothing else. Do NOT click "Start Over" or repeat any actions. Do NOT stop midway or assume you are done just because you filled out the input fields on a single page, if the request implies a multi-step workflow.
