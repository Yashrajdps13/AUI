...
| **[redux-flow](examples/redux-flow)** | Bridges Redux Toolkit global store slices, enforces read-only state rules, wraps dispatches to plain objects, and injects slice metadata for LLM planning guidance. | `cd examples/redux-flow` <br> `npm run dev` | `cd examples/redux-flow` <br> `python agent.py` |
