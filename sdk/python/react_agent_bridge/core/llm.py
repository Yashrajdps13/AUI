...
"""
    ...
    async def compile_goal(self, query: str, registry_snapshot: dict, original_query: Optional[str] = None) -> Goal:
        ...
        1. YOU MUST READ AND READ AGAIN the 'Active Registry Snapshot' and 'Allowed Target State Slots' list before compiling any goal. You can ONLY create goals/conditions targeting slots/components that exist in the registry snapshot (or are valid virtual page/component mounting targets). Do not guess or hallucinate any state slots or components.
        2. NEVER USE ERROR STATE SLOTS (such as target names ending with '.error' or containing 'error', e.g. 'LoginView.error' or 'LoginView#r5.error' being empty/null) OR DEBUG/CONSOLE LOGS (such as 'consoleLogs', 'appLog', 'logs') AS SUCCESS CONDITIONS. The absence of an error state or a log change is NOT a success signal and MUST be rejected. Instead, look for a positive outcome state in Zustand global stores or primary state slots (such as 'AuthStore.isAuthenticated' being true, or 'AuthStore.isLoggedIn' being true, or user details populated).
        3. For success_conditions and failure_conditions, the "target" field MUST be chosen strictly from the "Allowed Target State Slots" list. Do NOT guess, invent, or use any target path that is not present in that list.
        4. You MUST use the exact strings from "Allowed Target State Slots" as the target in your conditions. For example, if the list contains "App#r9.attendeeName", use exactly "App#r9.attendeeName", NOT "App.attendeeName" or "PassesStore.pass_holder_name".
        5. Do NOT include temporary input values or intermediate form fields (such as login password, search query, or form inputs) in the success_conditions if the user moves beyond that step or if the form/component unmounts or is reset. Instead, focus success conditions on final persistent state outcomes (e.g. user being authenticated in the AuthStore/Layout component, project list including the new project, task marked complete). Only include input values in success conditions if the goal is explicitly just to type a value and verify it remains visible on the current un-navigated screen.
        6. For navigation, routing, or page-transition goals where a target component/view is not yet mounted (and therefore its slots are missing from the registry snapshot), you MUST use virtual slots to check if the target component is mounted or the route is updated. Virtual slots use the target format:
        ...
        max_attempts = 3
        current_attempt = 1
        system_prompt = "You are a structured compiler. You MUST read the provided Active Registry Snapshot and Allowed Target State Slots first, and only compile goals targeting slots/components that are present in the registry snapshot or are valid virtual slots (ComponentName.isMounted or ComponentName.route). You are strictly prohibited from compiling success or failure conditions targeting slots that do not exist in the allowed list, unless they are virtual slots."
        user_prompt = prompt
